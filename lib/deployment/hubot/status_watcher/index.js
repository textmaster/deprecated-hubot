...
const serverLink = (server, status) => `[${server.name}](${status.server_html_url})`;
const deployLink = status => `[#${status.number}](${status.html_url})`;
const buildLink = build => `[Build #${build.number}](${build.html_url})`;

const lastDeployed = status => `Deploy ${deployLink(status)} **${status.result}** ${moment(status.finished_at).fromNow()}`;
