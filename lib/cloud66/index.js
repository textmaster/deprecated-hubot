...
const ACCESS_TOKEN =
  process.env.C66_PERSONAL_ACCESS_TOKEN || process.env.HUBOT_CLOUD66_AUTH_TOKEN;

const STACKS = [
  {name: 'test-2', id: 'feb44c30a7862357acd566c19795f6c8'},
...
async function listStacks() {
  const response = await fetch('https://app.cloud66.com/api/3/stacks.json', {
...
async function fetchEnvronments(stack_id) {
  const response = await fetch(
    `https://app.cloud66.com/api/3/stacks/${stack_id}/environments.json`,
    {headers: {Authorization: `Bearer ${ACCESS_TOKEN}`}},
  );
  const json = await response.json();
...
async function fetchDeployments(stack_id) {
  const response = await fetch(
    `https://app.cloud66.com/api/3/stacks/${stack_id}/deployments.json`,
    {headers: {Authorization: `Bearer ${ACCESS_TOKEN}`}},
  );
  const json = await response.json();
  // console.log("Environments", json)
