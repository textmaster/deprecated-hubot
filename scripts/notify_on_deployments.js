...
module.exports = function(robot) {
  ...
  const aggregateDeployedCommitsBetweenBuilds = async (
    deployment,
    previousBuild,
    latestBuild,
  ) => {
    ...
    return _.chain(builds)
      ...
      .filter(function(build) {
        return (
          build.build_number > previousBuildNumber &&
          build.build_number <= lastestBuildNumber
        );
      })
      .map(function(build) {
        return build.commit;
  ...
  const buildCommitMessage = commit => {
    return `* [${commit.id.substring(0, 8)}](${commit.url}) ${
      commit.message
  ...
  const buildMessage = (deployment, commits, events) => {
    ...
    // Title
    messages.push(
      `Semaphore ${deployment.result} to deployed on ${deployment.server_name}`,
    );
    // Commits
  ...
  robot.on('semaphore-deploy', async deployment => {
    if (deployment.server_name == 'production') {
      const previousDeployment = await fetchDeployment(
        deployment.project_hash_id,
        deployment.server_name,
        deployment.number - 1,
      );
      const previousBuild = await fetchBuildUsingBuildUrl(
        previousDeployment.build_url,
      );
      const latestBuild = await fetchBuild(
        deployment.project_hash_id,
        deployment.branch_name,
        deployment.build_number,
      );
      const commits = await aggregateDeployedCommitsBetweenBuilds(
        deployment,
        previousBuild,
        latestBuild,
      );
      ...
      Subscriptions.unsubscribeAllKeysFromEvent(
        `semaphore.${deployment.event}.${deployment.number}`,
      );
    }
  });
